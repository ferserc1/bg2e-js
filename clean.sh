#!/bin/sh

rm -rf examples/base/dist
rm -rf examples/base/node_modules
rm -rf examples/math/dist
rm -rf examples/math/node_modules
rm -rf examples/tools/dist
rm -rf examples/tools/node_modules
rm -rf examples/db/dist
rm -rf examples/db/node_modules
rm -rf examples/process_detection/dist
rm -rf examples/process_detection/node_modules
rm -rf examples/scene/dist
rm -rf examples/scene/node_modules
rm -rf examples/app/dist
rm -rf examples/app/node_modules
rm -rf examples/webgl/dist
rm -rf examples/webgl/node_modules
rm -rf examples/render_1/dist
rm -rf examples/render_1/node_modules
rm -rf examples/present_texture/dist
rm -rf examples/present_texture/node_modules
rm -rf examples/render_to_texture/dist
rm -rf examples/render_to_texture/node_modules
