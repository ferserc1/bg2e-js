...
import Bg2MouseEvent from "bg2e-js/ts/app/Bg2MouseEvent.js";
...
class RotateComponent extends Component {
    ...
    willUpdate(delta: number): void {
        if (this.transform && this._animation) {
            this.transform.matrix.rotate(0.5 * delta, 0, 1, 0);
            this.node!.postRedisplayFrames = 5;
        }
    }

    mouseUp(event: Bg2MouseEvent): void {
...
class MyAppController extends SceneAppController {
    ...
    async loadScene() {
        ...
        findVisitor.result.forEach(node => {
            ...
            materialModifier.applyDrawable(this.canvas, node.drawable!);
        });
